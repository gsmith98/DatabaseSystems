...
query5 = db.query().fromTable('lineitem').join(db.query().fromTable('orders'), method='block-nested-loops', \
  expr='L_ORDERKEY == O_ORDERKEY').join(db.query().fromTable('customer'), method='block-nested-loops', \
  expr='C_CUSTKEY == O_CUSTKEY').join(db.query().fromTable('supplier'), method='block-nested-loops', \
  expr='S_NATIONKEY == C_NATIONKEY and S_SUPPKEY == L_SUPPKEY').join(db.query().fromTable('nation'), method='block-nested-loops', \
  expr='S_NATIONKEY == N_NATIONKEY').join(db.query().fromTable('region'), method='block-nested-loops', \
  expr='N_REGIONKEY == R_REGIONKEY').where("R_NAME == 'ASIA' and O_ORDERDATE  >= 19940101 and O_ORDERDATE < 19950101" \
).groupBy( \
  groupSchema=DBSchema('group', [('N_NAME', 'text')]),
  aggSchema=DBSchema('sum', [('revenue', 'double')]), \
  groupExpr=(lambda e: e.N_NAME), \
  aggExprs=[(0, lambda acc, e: acc + (e.L_EXTENDEDPRICE * (1 - e.L_DISCOUNT)), lambda x: x)], \
  groupHashFn=(lambda e: e) \
).finalize() 
